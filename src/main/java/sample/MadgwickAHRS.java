...
public class MadgwickAHRS {
	...
	public float[] get360deg() {

		// Данный код был взят из библиотек FreeIMU
		float[] angles = new float[3];

		float gx = 2 * (quaternion[1] * quaternion[3] - quaternion[0] * quaternion[2]);
		float gy = 2 * (quaternion[0] * quaternion[1] + quaternion[2] * quaternion[3]);
		float gz = quaternion[0] * quaternion[0] - quaternion[1] * quaternion[1] - quaternion[2] * quaternion[2] + quaternion[3] * quaternion[3];

		// Курс (180 до -180).
		angles[0] = (float) Math.toDegrees((float) Math.atan2(2 * quaternion[1] * quaternion[2] - 2 * quaternion[0] * quaternion[3], 2 * quaternion[0] * quaternion[0] + 2 * quaternion[1] * quaternion[1] - 1));
		// Дифферент
